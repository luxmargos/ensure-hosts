...
export function resolveDefaultHostsPath(currentPlatform = platform(), env = process.env): string {
...
function tryWindowsPrivilegePrompt(options: ElevationOptions): ElevationResult {
  ...
  const result = spawnSync(
    'powershell.exe',
    ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', `$env:OUT = ${powerShellString(outputPath)} ; ${command}`],
    {
      cwd: options.cwd,
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'pipe'],
